 ...
 */
public class Main {
    // Turn on to log (expensive -- see Tranche:SimpleLog)
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Load the configuration for stress tests
...
//                TrancheServer ts = ConnectionUtil.connectHost(host, true);
    ...
    public static int runTest(int numFiles, long maxFileSize, String url, long minProjectSize, long maxProjectSize, boolean isUseBatch) {
        ...
        // Calculate the total of all uploads
        ...
        // Used to get the next projects size b/w min & max project size =)
        long projectCount = 0;
        ...
        try {
            ...
            while (totalBytesUploaded < totalBytesToUpload) {
                ...
                // If same size, just give that and be done
                if (minProjectSize == maxProjectSize) {
                    nextProjectSize = minProjectSize;
                } else {
                    // Yuck, need the difference as an integer
                    ...
                    // HAck to get a better distribution =)
...
//                System.out.println("DEBUG> Using project: " + project.getName() + " (" + Text.getFormattedDate(System.currentTimeMillis()) + ")");
...
//                BigHash hash = aft.addFile(project);
            ...
            // Use stdout -- suspicious may be losing stderr
