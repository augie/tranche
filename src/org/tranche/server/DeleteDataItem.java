 ...
 */
public class DeleteDataItem extends ServerItem {

    /**
     * @param   server  the server received
     */
    public DeleteDataItem(Server server) {
        super(Token.DELETE_DATA_STRING, server);
    }
    ...
    public final void doAction(InputStream in, OutputStream out, String clientIP) throws Exception {
        ...
        try {
            if (hostMap.containsKey(server.getHostName())) {
                ...
                try {
                    exceptions.addAll(server.getTrancheServer().deleteData(hash, new Signature[]{thisServerWrapper.sig}, nonces, new String[]{server.getHostName()}).getErrors());
                } catch (Exception e) {
                    ...
                    Thread t = new Thread(hostToContact) {
                        ...
                        public void run() {
                            try {
                                ...
                                exceptions.addAll(ts.deleteData(hash, sigsSubsetArr, noncesSubsetArr, hostsSubset).getErrors());
                            } catch (Exception e) {
     ...
     */
    public static final void writeRequest(boolean writeHeader, BigHash hash, String[] hosts, Signature[] signatures, byte[][] nonces, OutputStream out) throws Exception {
        if (writeHeader) {
            RemoteUtil.writeBytes(Token.DELETE_DATA, out);
        }
