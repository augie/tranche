 ...
 */
public class SetMetaDataItem extends ServerItem {

    /**
     * @param   server  the server received
     */
    public SetMetaDataItem(Server server) {
        super(Token.SET_META_STRING, server);
    }
    ...
    public final void doAction(InputStream in, OutputStream out, String clientIP) throws Exception {
        ...
        final Signature sig;
        try {
            merge = RemoteUtil.readBoolean(in);
        ...
        try {
            // Is this server in the target collection?
            if (hostsSet.contains(server.getHostName())) {
                hostsSet.remove(server.getHostName());
                try {
                    exceptions.addAll(server.getTrancheServer().setMetaData(merge, hash, data, sig, new String[]{server.getHostName()}).getErrors());
                } catch (Exception e) {
                                ...
                                try {
                                    ...
                                    if (TestUtil.isTestingManualNetworkStatusTable()) {
                                        ...
                                        for (String nextOstensibleHost : partitionRecipients) {
                                            ...
                                            PropagationReturnWrapper nextWrapper = ffts.setMetaData(merge, hash, data, sig, hostArr);
                                            exceptionWrappers.addAll(nextWrapper.getErrors());
                                        }

                                        prw = new PropagationReturnWrapper(exceptionWrappers);
                                    } else {
                                        prw = ts.setMetaData(merge, hash, data, sig, strategy.getPartitionsMap().get(hostToContact).toArray(new String[0]));
                                    }
     ...
     */
    public static final void writeRequest(boolean writeHeader, boolean merge, BigHash hash, String[] hosts, byte[] bytes, Signature signature, OutputStream out) throws Exception {
        if (writeHeader) {
            RemoteUtil.writeBytes(Token.SET_META, out);
        }
        RemoteUtil.writeBoolean(merge, out);
